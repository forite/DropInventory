# Unavoidable repeated commands for all of these, unfortunately
# so only this one will be commented
...
# Cleans up and gives the rest of the item instances
data remove storage dinv:storage Query[0]
execute if data storage dinv:storage Query[0] run function dinv:private/give_next

# Removes all original item instances and saves possibly overwritten items
